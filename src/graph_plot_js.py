...
def tourny_html_code(
    ...
    # Node size is proportional to the out-degree, scaled by ``vertex_size``.
    # The '+ 1' ensures that vertices with out-degree 0 remain visible.
    nodes = [
        {
            "name": str(v),
            "group": str(color[v_to_id[v]]),
            "size": (G.out_degree(v) + 1) * int(vertex_size),
        }
        for v in G
    ]
    ...
    with open(Path(__file__).parent / "graph_plot_js.html") as f:
